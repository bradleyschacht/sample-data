 ...
 order by d_week_seq1
    ...
    /*************************************   TPC-DS Query 09   *************************************/
        ...
        select case when (select count_big(*) /* select case when (select count(*) */
                          from store_sales 
                          where ss_quantity between 1 and 20) > 3496203
        ...
        case when (select count_big(*) /* case when (select count(*) */
                          from store_sales
                          where ss_quantity between 21 and 40) > 3708875
                    then (select avg(ss_ext_list_price)
                          from store_sales
        ...
        case when (select count_big(*) /* case when (select count(*) */
                          from store_sales
                          where ss_quantity between 41 and 60) > 2342264
                    then (select avg(ss_ext_list_price)
                          from store_sales
    ...
    /*************************************   TPC-DS Query 16   *************************************/
        ...
        where
            d_date between '2000-3-01' and 
                   (dateadd(day, + 60, cast('2000-3-01' as date))) /*  (cast('2000-3-01' as date) + 60 days)  */
        and cs1.cs_ship_date_sk = d_date_sk
        and cs1.cs_ship_addr_sk = ca_address_sk
        and ca_state = 'KS'
        and cs1.cs_call_center_sk = cc_call_center_sk
        and cc_county in ('Luce County','Ziebach County','Walker County','Barrow County',
                          'Franklin Parish'
        )
        and exists (select *
        ...
        select top 100 c_last_name
         ...
         from
           (select ss_ticket_number
            ...
            and store.s_city in ('New Hope','Greenwood','Centerville','Union','Five Points') 
