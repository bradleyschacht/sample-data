...
*/
    ...
    /*************************************   TPC-DS Query 02   *************************************/
        ...
        from catalog_sales) AS x), /* from catalog_sales)), */
         wswscs as 
         (select d_week_seq,
 ...
 order by d_week_seq1
    ...
    /*************************************   TPC-DS Query 05   *************************************/
        ...
        ;with ssr as
         ...
         where date_sk = d_date_sk
               and d_date between cast('1998-08-17' as date) 
                          and (dateadd(day, +  14, cast('1998-08-17' as date))) /*  and (cast('1998-08-17' as date) +  14 days)  */
        ...
        , 'store' + s_store_id as id /* , 'store'  +  s_store_id as id /*  , 'store' || s_store_id as id  */ */
                , sales
                , returns
        ...
        , 'catalog_page' + cp_catalog_page_id as id /* , 'catalog_page'  +  cp_catalog_page_id as id /*  , 'catalog_page' || cp_catalog_page_id as id  */ */
                , sales
                , returns
                , (profit - profit_loss) as profit
         from  csr
        ...
        , 'web_site' + web_site_id as id /* , 'web_site'  +  web_site_id as id /*  , 'web_site' || web_site_id as id  */ */
                , sales
                , returns
                , (profit - profit_loss) as profit
         from   wsr
    ...
    /*************************************   TPC-DS Query 16   *************************************/
        ...
        where
            d_date between '1999-4-01' and 
                   (dateadd(day, + 60, cast('1999-4-01' as date))) /*  (cast('1999-4-01' as date) + 60 days)  */
        and cs1.cs_ship_date_sk = d_date_sk
        and cs1.cs_ship_addr_sk = ca_address_sk
        and ca_state = 'MS'
        and cs1.cs_call_center_sk = cc_call_center_sk
        and cc_county in ('Ziebach County','Richland County','Bronx County','Barrow County',
                          'Fairfield County'
        )
        and exists (select *
 ...
 order by store_q1_q2_increase
    ...
    /*************************************   TPC-DS Query 34   *************************************/
        ...
        select c_last_name
           ...
           (select ss_ticket_number
            ...
            and store.s_county in ('Ziebach County','Richland County','Bronx County','Barrow County',
                                   'Fairfield County','Williamson County','Luce County','Franklin Parish')
        ...
        select top 100 c_last_name
         ...
         from
           (select ss_ticket_number
            ...
            and store.s_city in ('Centerville','Pleasant Hill','Oakland','Pleasant Grove','Fairview') 
        ...
        cross_sales as
         (select i_product_name product_name
          ...
          WHERE  ss_store_sk = s_store_sk AND
                 ...
                 i_color in ('moccasin','bisque','rosy','dim','blue','ivory') and
    ...
    /*************************************   TPC-DS Query 66   *************************************/
        ...
        select top 100  
         ...
         from (
         	...
         	,w_state
         	,w_country
         	,'AIRBORNE'  +  ','  +  'HARMSTORF' as ship_carriers /*  ,'AIRBORNE' || ',' || 'HARMSTORF' as ship_carriers  */
               ,d_year as year
         	...
         	and ws_ship_mode_sk = sm_ship_mode_sk
                and d_year = 2002
         	and t_time between 51601 and 51601+28800 
    ...
    /*************************************   TPC-DS Query 72   *************************************/
        ...
        and d3.d_date > dateadd(day, 5, d1.d_date) /* and d3.d_date > d1.d_date + 5 */
    ...
    /*************************************   TPC-DS Query 77   *************************************/
        ...
        ;with ss as
         ...
         where ss_sold_date_sk = d_date_sk
               and d_date between cast('1998-08-17' as date) 
                          and (dateadd(day, +  30, cast('1998-08-17' as date)))  /*  and (cast('1998-08-17' as date) +  30 days)  */
               and ss_store_sk = s_store_sk
         group by s_store_sk)
         ,
         sr as
    ...
    /*************************************   TPC-DS Query 80   *************************************/
        ...
        group by web_site_id)
         ...
         from 
         (select 'store channel' as channel
                , 'store'  +  store_id as id /*  , 'store' || store_id as id  */
                , sales
                , returns
                , profit
         from   ssr
         union all
         select 'catalog channel' as channel
                , 'catalog_page'  +  catalog_page_id as id /*  , 'catalog_page' || catalog_page_id as id  */
                , sales
                , returns
                , profit
         from  csr
