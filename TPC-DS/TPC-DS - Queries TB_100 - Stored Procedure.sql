...
BEGIN
	...
	/*************************************   Notes   *************************************/
	/*
		John Hoang 8/20/2024. 
		This is the TPC-DS 100TB SF queries modified for Fabric DW (days, subquery alias, substring, stddev_samp, and rowcount modification)
		12 queries have different predicates compared to 1TB queries.1,9,16,27,33,34,36,33,44,46,68,73
	*/
	...
	and --(cast('1999-3-01' as date) + 60 days)
	dateadd(
		day, 
		60, 
		cast('1999-3-01' as date)
	) 
	...
	and cc_county in (
		'Perry County', 'Walker County', 
		'Barrow County', 'San Miguel County', 
		'Jackson County'
	) 
	and exists (
		select 
	...
	and d_year = 2001 
	and s_state in ('WV', 'MI', 'OK', 'WA', 'TX', 'PA') 
	group by 
	...
	(
		...
		and store.s_county in (
			'Clinton County', 'Amherst County', 
			'Musselshell County', 'Miner County', 
			'Wolfe County', 'Big Horn County', 
			'Terry County', 'Coal County'
		) 
		group by 
		ss_ticket_number, 
		ss_customer_sk
	...
	from (select ss_ticket_number
		...
		where store_sales.ss_sold_date_sk = date_dim.d_date_sk
			...
			and date_dim.d_year in (2000,2000+1,2000+2)
			and store.s_city in ('Richland','Elmwood')
	...
	(select ss_ticket_number
		...
		and date_dim.d_year in (1998,1998+1,1998+2)
		and store.s_county in ('Clinton County','Amherst County','Musselshell County','Miner County')
